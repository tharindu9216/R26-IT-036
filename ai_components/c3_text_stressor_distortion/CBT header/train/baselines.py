"""Machine learning baseline models for CDT (cognitive distortion) classification.
...
transformer-based models. Unlike the Stress header (binary), this is an
11-class multiclass problem — both LogisticRegression and
CalibratedClassifierCV(LinearSVC) support multiclass predict_proba natively,
so no one-vs-rest wrapping is needed here.
"""
...
from utils  import compute_hierarchical_metrics


def train_baseline(name, full_df, test_df, logger=None):
    """
    Train one ML baseline.
    Uses raw 'Patient Question' column — exactly as saved by preprocessing.
    ...
    # Raw text (preprocessing saves this as 'Patient Question')
    X_train = full_df['Patient Question'].fillna('').tolist()
    X_test  = test_df['Patient Question'].fillna('').tolist()
    ...
    # Evaluate — predict_proba returns the full (N, 11) matrix for multiclass
    preds = clf.predict(X_test_v)
    probs = clf.predict_proba(X_test_v)
    metrics = compute_hierarchical_metrics(
        y_test.tolist(), preds.tolist(), probs.tolist())

    if logger:
        logger.log(f'  Test Accuracy : {metrics["accuracy"]:.4f}')
        logger.log(f'  Test F1 Macro : {metrics["f1_macro"]:.4f}')
        logger.log(f'  Binary F1     : {metrics["binary_f1_macro"]:.4f}')
        logger.log(f'  Type F1       : {metrics["type_f1_macro"]:.4f}')
