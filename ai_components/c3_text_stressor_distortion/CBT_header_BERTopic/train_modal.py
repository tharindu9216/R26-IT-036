"""Run CBT Header BERTopic training on a Modal GPU."""
...
def find_project_root():
    ...
    for candidate in (here.parent, *here.parents):
        if (
            ...
            return candidate
    remote_root = Path('/root/cbt_bertopic')
...
modal_gpu = (
    load_env_value('CBT_BERTOPIC_GPU')
    or load_env_value('BERTOPIC_GPU')
...
volume = modal.Volume.from_name(
    'cbt-bertopic-outputs', create_if_missing=True)
app = modal.App('cbt-header-bertopic')
...
def run_training():
    ...
    remote_root = '/root/cbt_bertopic'
    train_dir = (
        remote_root
        + '/ai_components/c3_text_stressor_distortion/'
        + 'CBT_header_BERTopic'
    )
    data_dir = remote_root + '/data/CBT header/processed'
    output_dir = '/results/cbt_bertopic_outputs'
    os.environ['DATA_DIR'] = data_dir
    os.environ['OUTPUT_DIR'] = output_dir
    os.environ['CBT_BERTOPIC_BATCH_SIZE'] = str(batch_size)
    os.makedirs(output_dir, exist_ok=True)

    required = [
        os.path.join(data_dir, 'cbt_train.csv'),
        os.path.join(data_dir, 'cbt_val.csv'),
        os.path.join(data_dir, 'cbt_test.csv'),
    ]
    for path in required:
        if not os.path.exists(path):
            raise FileNotFoundError(f'Missing CBT data: {path}')
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f'CBT BERTopic failed with exit code {result.returncode}. '
            'Partial outputs were committed to cbt-bertopic-outputs.'
        )
...
def main():
    print(f'Submitting CBT BERTopic to Modal GPU: {modal_gpu}')
    result = run_training.spawn().get()
    print(
        f'Done: {result["topics"]} topics in '
        f'{result["minutes"]} minutes')
    print(
        'Download: modal volume get cbt-bertopic-outputs '
