"""Train a leakage-safe BERTopic model for Stress Header documents."""
...
def build_logger(output_dir):
    ...
    logger = logging.getLogger('stress_bertopic')
...
def load_splits(data_dir):
    """Load and validate the original Stress Header processed splits."""
    frames = {}
    for split, filename in Config.SPLIT_FILES.items():
        path = Path(data_dir) / filename
        if not path.exists():
            raise FileNotFoundError(f'Missing Stress split: {path}')
        ...
        if (frame[Config.TEXT_COLUMN] == '').any():
            count = int((frame[Config.TEXT_COLUMN] == '').sum())
            raise ValueError(
                f'{path.name} contains {count} empty documents')
        frame['split'] = split
...
def prepare_fit_documents(frames):
    """Exclude recruitment posts and exact repeats from topic discovery."""
    for frame in frames.values():
        frame['is_research_boilerplate'] = frame[
            Config.TEXT_COLUMN].map(is_research_boilerplate)
    ...
    candidates['is_duplicate_for_topic_fit'] = normalized.duplicated(
        keep='first')
    ...
    candidates['excluded_from_topic_fit'] = (
        excluded_boilerplate | excluded_duplicate)
    candidates['topic_fit_exclusion_reason'] = np.select(
        [
            excluded_boilerplate & excluded_duplicate,
            excluded_boilerplate,
            excluded_duplicate,
        ],
        ['research_boilerplate+duplicate',
         'research_boilerplate', 'duplicate'],
        default='',
    )
...
def _package_assignments(frame, topics, probabilities, topic_names):
    ...
    assigned.insert(
        ...
        [
            f'{split}_{row}'
            for split, row in zip(
                assigned['split'], assigned['source_row'])
        ],
    )
    assigned['stress_label'] = assigned['label'].map({
        0: 'Not Stressed',
        1: 'Stressed',
    }).fillna('Unknown')
...
def main():
    ...
    logger.info('Stress Header BERTopic training started')
    ...
    logger.info(
        ...
        '(%s recruitment boilerplate, %s duplicates detected)',
    ...
    fit_documents = fit_frame[Config.TEXT_COLUMN].tolist()
    ...
    topic_label_summary = (
        document_topics.groupby([
            'topic', 'topic_name', 'stress_label'], as_index=False)
        .size()
        .rename(columns={'size': 'document_count'}))
    topic_label_summary['percentage_within_topic'] = (
        topic_label_summary['document_count']
        / topic_label_summary.groupby('topic')['document_count']
        .transform('sum'))
    topic_label_summary.to_csv(
        Config.OUTPUT_DIR / 'topic_by_stress_label.csv', index=False)

    topic_subreddit_summary = (
        document_topics.groupby([
            'topic', 'topic_name', 'subreddit'], as_index=False)
        .size()
        .rename(columns={'size': 'document_count'}))
    topic_subreddit_summary.to_csv(
        Config.OUTPUT_DIR / 'topic_by_subreddit.csv', index=False)
    ...
    save_json({
        'task': 'Stress Header unsupervised topic discovery',
